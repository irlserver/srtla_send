...
/// selection.
///
/// Also refreshes each link's `conn_timeout_ms`, reconnect fast-retry window
/// and `delay_budget_ms` from the snapshot, so the runtime-tunable liveness window and the peer's declared
