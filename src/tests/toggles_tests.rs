...
mod tests {
    ...
    fn test_dynamic_toggles_new() {
        ...
        assert!(toggles.stickiness_enabled.load(Ordering::Relaxed));
    ...
    fn test_dynamic_toggles_from_cli() {
        // Test all defaults false/true
        let toggles = DynamicToggles::from_cli(false, false, false, false);
        assert!(!toggles.classic_mode.load(Ordering::Relaxed));
        assert!(toggles.stickiness_enabled.load(Ordering::Relaxed)); // !no_stickiness
        assert!(toggles.quality_scoring_enabled.load(Ordering::Relaxed)); // !no_quality
        assert!(!toggles.exploration_enabled.load(Ordering::Relaxed));

        // Test various combinations
        let toggles = DynamicToggles::from_cli(true, true, true, true);
        assert!(toggles.classic_mode.load(Ordering::Relaxed));
        assert!(!toggles.stickiness_enabled.load(Ordering::Relaxed)); // !no_stickiness = !true = false
        assert!(!toggles.quality_scoring_enabled.load(Ordering::Relaxed)); // !no_quality = !true = false
        assert!(toggles.exploration_enabled.load(Ordering::Relaxed));
    }
    ...
    #[test]
    fn test_apply_cmd_stickiness() {
        let toggles = DynamicToggles::new();

        crate::toggles::apply_cmd(&toggles, "stick off");
        assert!(!toggles.stickiness_enabled.load(Ordering::Relaxed));

        crate::toggles::apply_cmd(&toggles, "stick on");
        assert!(toggles.stickiness_enabled.load(Ordering::Relaxed));

        // Test alternative format
        crate::toggles::apply_cmd(&toggles, "stickiness=false");
        assert!(!toggles.stickiness_enabled.load(Ordering::Relaxed));

        crate::toggles::apply_cmd(&toggles, "stickiness=true");
        assert!(toggles.stickiness_enabled.load(Ordering::Relaxed));
    }
    ...
    fn test_apply_cmd_status() {
        ...
        // Status command should not panic and should not change any values
        let classic_before = toggles.classic_mode.load(Ordering::Relaxed);
        let stick_before = toggles.stickiness_enabled.load(Ordering::Relaxed);
    ...
    fn test_apply_cmd_whitespace_handling() {
        ...
        // Commands with extra whitespace should work
        crate::toggles::apply_cmd(&toggles, "  classic on  ");
        assert!(toggles.classic_mode.load(Ordering::Relaxed));
    ...
    fn test_all_command_formats() {
        ...
        let test_commands = vec![
            ...
            ("stick on", "stick off"),
            ("stickiness=true", "stickiness=false"),
        ...
        for (on_cmd, off_cmd) in test_commands {
            ...
            let initial_stick = toggles.stickiness_enabled.load(Ordering::Relaxed);
