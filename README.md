...
- `--conn-timeout-ms <MS>`: Per-link liveness timeout; silence past this tears the link down and re-registers it (default 5000, clamped 1000–60000; runtime: `set_conn_timeout`)
- `--reconnect-fast-retry-ms <MS>`: Reconnect cadence for an uplink that was established and has timed out, during its fast-retry window (default 1000, clamped 1000–5000; runtime: `set_reconnect_fast_retry`). Matches the C reference, which re-opens the socket on every 1 s housekeeping pass, so a short modem/tether blip is re-registered before the SRT session's peer-idle timeout (5 s) ends it
- `--reconnect-fast-retry-attempts <N>`: Failed reconnects retried at `--reconnect-fast-retry-ms` before the exponential backoff (5 s doubling, 120 s cap) takes over (default 4, max 10). `0` restores the pre-window behaviour: backoff from the first miss
